...
impl IFooInterface for BpFooInterface {
  ...
  fn r#newApi<'a, >(&'a self) -> binder::Result<()> {
    ...
    let _aidl_reply = self.binder.submit_transact(transactions::r#newApi, _aidl_data, {#[cfg(any(android_vndk, not(android_ndk)))] { binder::binder_impl::FLAG_PRIVATE_LOCAL }#[cfg(not(any(android_vndk, not(android_ndk))))] { 0 }});
...
impl<P: binder::BinderAsyncPool> IFooInterfaceAsync<P> for BpFooInterface {
  ...
  fn r#newApi<'a, >(&'a self) -> binder::BoxFuture<'a, binder::Result<()>> {
    ...
    let binder = self.binder.clone();
    P::spawn(
      move || binder.submit_transact(transactions::r#newApi, _aidl_data, {#[cfg(any(android_vndk, not(android_ndk)))] { binder::binder_impl::FLAG_PRIVATE_LOCAL }#[cfg(not(any(android_vndk, not(android_ndk))))] { 0 }}),
      move |_aidl_reply| async move {
        self.read_response_newApi(_aidl_reply)
      }
    )
