...
impl INamedCallback for BpNamedCallback {
  fn r#GetName<'a, >(&'a self) -> binder::Result<String> {
    ...
    let _aidl_reply = self.binder.submit_transact(transactions::r#GetName, _aidl_data, if cfg!(any(android_vndk, not(android_ndk))) { binder::binder_impl::FLAG_PRIVATE_LOCAL } else { 0 });
...
impl<P: binder::BinderAsyncPool> INamedCallbackAsync<P> for BpNamedCallback {
  fn r#GetName<'a, >(&'a self) -> binder::BoxFuture<'a, binder::Result<String>> {
    ...
    P::spawn(
      move || binder.submit_transact(transactions::r#GetName, _aidl_data, if cfg!(any(android_vndk, not(android_ndk))) { binder::binder_impl::FLAG_PRIVATE_LOCAL } else { 0 }),
